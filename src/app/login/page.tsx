...
function LoginPageContent() {
    ...
    const {
        ...
        "auth/email-login",
        async (_key, { arg }: { arg: LoginFormInputs }) => {
            try {
    ...
    return (
        <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-12 sm:px-6 lg:px-8">
            ...
            <div className="relative z-10 w-full max-w-105 space-y-8">
                ...
                <div className="rounded-[2.5rem] border border-white/20 bg-white/95 p-8 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-10">
                    {(successMsg || currentError) && (
                        <div
                            className={`mb-6 flex items-start gap-3 rounded-2xl border p-4 text-sm font-semibold ${successMsg
                        ...
                        >
                            ...
                            <span>{successMsg || currentError}</span>
                        </div>
                    )}
                    ...
                    <div className="space-y-6">
                        <button
                            ...
                            disabled={isLoading}
                            className="relative flex w-full items-center justify-center gap-3 rounded-2xl border border-gray-200 bg-white/80 px-4 py-3.5 text-sm font-bold text-gray-700 transition-all hover:-translate-y-0.5 hover:bg-white hover:shadow-md focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none disabled:opacity-70 disabled:hover:translate-y-0"
                        >
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                            <div>
                                ...
                                <div className="group relative">
                                    <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                                        <Mail className="h-5 w-5 text-gray-400 transition-colors group-focus-within:text-indigo-600" />
                                    </div>
                                    <input
                                        type="email"
                                        autoComplete="email"
                                        ...
                                        className="block w-full rounded-2xl border border-gray-200 bg-white py-3.5 pr-4 pl-12 text-sm font-medium text-gray-900 outline-none transition-all placeholder:text-gray-400 focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10"
                            ...
                            <div>
                                ...
                                <div className="group relative">
                                    <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                                        <Lock className="h-5 w-5 text-gray-400 transition-colors group-focus-within:text-indigo-600" />
                                    </div>
                                    <input
                                        type="password"
                            ...
                            <div className="space-y-6 pt-2">
                                <label className="group flex cursor-pointer items-center gap-3">
                                    <div className="relative flex items-center">
                                        <input
                                            type="checkbox"
                                            className="peer h-5 w-5 cursor-pointer appearance-none rounded-lg border-2 border-gray-300 transition-all hover:border-indigo-600 checked:border-indigo-600 checked:bg-indigo-600"
                                            {...register("rememberMe")}
                                            disabled={isLoading}
                                        />
                                        <CheckCircle2 className="pointer-events-none absolute top-1/2 left-1/2 h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 text-white opacity-0 transition-opacity peer-checked:opacity-100" />
                                    </div>
                                ...
                                <button
                                    type="submit"
                                    disabled={isLoading}
                                    className="group relative flex w-full items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-4 py-4 text-sm font-bold text-white shadow-xl shadow-indigo-600/30 transition-all hover:-translate-y-0.5 hover:bg-indigo-700 disabled:opacity-70 disabled:hover:translate-y-0"
                                >
                                    {isEmailLoading ? (
                                        <>
                ...
                <p className="text-center text-sm font-medium text-gray-300">
                    ...
                    <Link
                        href="/register"
                        className="font-bold text-white transition-colors hover:text-indigo-300 hover:underline"
                    >
                        Hemen Kayıt Olun
                    </Link>
                </p>
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-gray-950 text-white">
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-6 py-4">
                <Loader2 className="h-5 w-5 animate-spin" />
                <span>Giriş sayfası yükleniyor...</span>
            </div>
        </div>
    );
}
